...
#include "tzmap-timezone-map.h"
#include <math.h>
#include <string.h>
#include "tz.h"

G_DEFINE_TYPE (TzmapTimezoneMap, tzmap_timezone_map, GTK_TYPE_WIDGET)

#define TIMEZONE_MAP_PRIVATE(o) \
  (G_TYPE_INSTANCE_GET_PRIVATE ((o), TZMAP_TYPE_TIMEZONE_MAP, TzmapTimezoneMapPrivate))
...
typedef struct
{
  ...
  guchar blue;
  guchar alpha;
} TzmapTimezoneMapOffset;

struct _TzmapTimezoneMapPrivate
{
...
static TzmapTimezoneMapOffset color_codes[] =
{
    {-11.0, 43, 0, 0, 255 },
    {-10.0, 85, 0, 0, 255 },
    {-9.5, 102, 255, 0, 255 },
...
static void
tzmap_timezone_map_get_property (GObject    *object,
                              guint       property_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
...
static void
tzmap_timezone_map_set_property (GObject      *object,
                              guint         property_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
...
static void
tzmap_timezone_map_dispose (GObject *object)
{
  TzmapTimezoneMapPrivate *priv = TZMAP_TIMEZONE_MAP (object)->priv;
  ...
  G_OBJECT_CLASS (tzmap_timezone_map_parent_class)->dispose (object);
}

static void
tzmap_timezone_map_finalize (GObject *object)
{
  TzmapTimezoneMapPrivate *priv = TZMAP_TIMEZONE_MAP (object)->priv;
  ...
  G_OBJECT_CLASS (tzmap_timezone_map_parent_class)->finalize (object);
}

/* GtkWidget functions */
static void
tzmap_timezone_map_get_preferred_width (GtkWidget *widget,
                                     gint      *minimum,
                                     gint      *natural)
{
...
static void
tzmap_timezone_map_get_preferred_height (GtkWidget *widget,
                                      gint      *minimum,
                                      gint      *natural)
{
  TzmapTimezoneMapPrivate *priv = TZMAP_TIMEZONE_MAP (widget)->priv;
  gint size;
...
static void
tzmap_timezone_map_size_allocate (GtkWidget     *widget,
                               GtkAllocation *allocation)
{
  ...
  GTK_WIDGET_CLASS (tzmap_timezone_map_parent_class)->size_allocate (widget,
                                                                  allocation);
}

static void
tzmap_timezone_map_realize (GtkWidget *widget)
{
...
static gboolean
tzmap_timezone_map_draw (GtkWidget *widget,
                      cairo_t   *cr)
{
  TzmapTimezoneMapPrivate *priv = TZMAP_TIMEZONE_MAP (widget)->priv;
...
static void
tzmap_timezone_map_state_flags_changed (GtkWidget     *widget,
                                     GtkStateFlags  prev_state)
{
  update_cursor (widget);

  if (GTK_WIDGET_CLASS (tzmap_timezone_map_parent_class)->state_flags_changed)
    GTK_WIDGET_CLASS (tzmap_timezone_map_parent_class)->state_flags_changed (widget, prev_state);
}


static void
tzmap_timezone_map_class_init (TzmapTimezoneMapClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  g_type_class_add_private (klass, sizeof (TzmapTimezoneMapPrivate));

  object_class->get_property = tzmap_timezone_map_get_property;
  object_class->set_property = tzmap_timezone_map_set_property;
  object_class->dispose = tzmap_timezone_map_dispose;
  object_class->finalize = tzmap_timezone_map_finalize;

  widget_class->get_preferred_width = tzmap_timezone_map_get_preferred_width;
  widget_class->get_preferred_height = tzmap_timezone_map_get_preferred_height;
  widget_class->size_allocate = tzmap_timezone_map_size_allocate;
  widget_class->realize = tzmap_timezone_map_realize;
  widget_class->draw = tzmap_timezone_map_draw;
  widget_class->state_flags_changed = tzmap_timezone_map_state_flags_changed;
...
static void
set_location (TzmapTimezoneMap *map,
              TzLocation    *location)
{
  TzmapTimezoneMapPrivate *priv = map->priv;
  TzInfo *info;
                    ...
                    GdkEventButton *event)
{
  ...
  set_location (TZMAP_TIMEZONE_MAP (widget), (TzLocation*) distances->data);
...
static void
tzmap_timezone_map_init (TzmapTimezoneMap *self)
{
  ...
  priv->orig_color_map = gdk_pixbuf_new_from_file (DATADIR "/tzmap.png",
                                                   &err);
  if (!priv->orig_color_map)
    {
      g_warning ("Could not load background image: %s",
...
/**
 * tzmap_timezone_map_new:
 *
 * Create a new timezone map
 *
 * Returns: a new #TzmapTimezoneMap
 */
TzmapTimezoneMap *
tzmap_timezone_map_new (void)
{
  return g_object_new (TZMAP_TYPE_TIMEZONE_MAP, NULL);
}

gboolean
tzmap_timezone_map_set_timezone (TzmapTimezoneMap *map,
...
TzLocation *
tzmap_timezone_map_get_location (TzmapTimezoneMap *map)
{
